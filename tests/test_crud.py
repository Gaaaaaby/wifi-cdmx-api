"""
Pruebas unitarias para operaciones CRUD.
"""
...
class TestGetNearbyPaginated:
    def test_returns_points_ordered_by_distance(self, db_session, sample_wifi_points):
        ...
        assert result["total"] == len(sample_wifi_points)
        # Verificar que el primer punto es el mas cercano
    ...
    def test_includes_distance_in_metros(self, db_session, sample_wifi_points):
        ...
        lat, lng = 19.432707, -99.086743
        result = wifi_crud.get_nearby_paginated(db_session, lat, lng, page=1, limit=10)
        # Verificar que distancia existe y es un numero positivo o None
        if result["data"]:
            distancia = result["data"][0].get("distancia") if isinstance(result["data"][0], dict) else getattr(result["data"][0], "distancia", None)
    ...
    # La funcion deberia manejar coordenadas invalidas sin crashear
        try:
            result = wifi_crud.get_nearby_paginated(db_session, lat=200, lng=-99, page=1, limit=10)
